...
@Component({
  selector: 'app-posts-nintendo-switch-2',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './posts-nintendo-switch-2.html',
  styleUrls: ['./posts-nintendo-switch-2.css']
})
export class PostsNintendoSwitch2 implements OnInit {
  posts = signal<any[]>([]);
  userId = signal<string>('');
  userName = signal<string>('');
  
  // Manejo de comentarios por ID de post
  ...
  async ngOnInit() {
    // 1. Datos del usuario para comentar y votar
    ...
    // 2. Cargar noticias de Switch 2
    this.cargarPosts();
  }

  async cargarPosts() {
    const { data } = await this.postRepo.obtenerPostsPorCategoria('Switch 2');
    if (data) {
      this.posts.set(data);
    }
  }
